...
import travelAdvisoriesData from '@/data/mock/travel-advisories.json'
...
const mockDataMap: Record<string, any> = {
  ...
  'travel-advisories.json': travelAdvisoriesData,
  'news.json': newsData,
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetTravelAdvisories') || query.includes('nodeTravelAdvisories')) {
      return loadMockData('travel-advisories.json')
    }
