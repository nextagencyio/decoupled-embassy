...
interface TravelAdvisoryCardProps {
  item: DrupalTravelAdvisory
}

export default function TravelAdvisoryCard({ item }: TravelAdvisoryCardProps) {
  return (
    <Link
      href={item.path || '#'}
    ...
    >
      ...
      <div className="p-6">
          {(item as any).country && (
            <p className="text-sm text-blue-700 font-medium mb-2">{(item as any).country}</p>
          )}
