...
import { GET_TRAVEL_ADVISORY_BY_PATH } from '@/lib/queries'
...
interface TravelAdvisoryByPathData {
  route: {
    entity: DrupalTravelAdvisory
  } | null
}

async function getTravelAdvisory(path: string): Promise<DrupalTravelAdvisory | null> {
  try {
    const requestHeaders = await headers()
    const apolloClient = getServerApolloClient(requestHeaders)
    const { data } = await apolloClient.query<TravelAdvisoryByPathData>({
      query: GET_TRAVEL_ADVISORY_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/travel-advisories/${slug.join('/')}`
  const item = await getTravelAdvisory(path)

  if (!item) {
    return { title: 'Travel Advisory Not Found | Embassy' }
  }

  return {
    title: `${item.title} | Embassy`,
    description: ((item as any).body?.processed ? (item as any).body.processed.replace(/<[^>]*>/g, '').substring(0, 160) : '') || `Learn more about ${item.title}.`,
  }
}

export default async function TravelAdvisoryDetailPage({ params }: PageProps) {
  const { slug } = await params
  const path = `/travel-advisories/${slug.join('/')}`
  const item = await getTravelAdvisory(path)
